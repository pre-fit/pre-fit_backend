...
class Shop_Items_Model(Base):
    ...
    item_price: Mapped[int] = mapped_column(Integer)																# 상품가격
    item_discount_price: Mapped[int] = mapped_column(Integer)														# 상품할인가격
    item_create_date: Mapped[datetime] = mapped_column(DateTime)															# 상품등록일
    item_soldout: Mapped[datetime] = mapped_column(Boolean)																# 품절여부

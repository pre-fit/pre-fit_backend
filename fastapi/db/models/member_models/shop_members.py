...
class Shop_Members_Model(Base):
    ...
    member_gender: Mapped[bool] = mapped_column(Boolean)  									# 회원성별
    member_birthday: Mapped[datetime] = mapped_column(DateTime)  								# 회원생일
    member_point: Mapped[int] = mapped_column(Integer)  									# 회원포인트
    member_email: Mapped[str] = mapped_column(String(length=256)) 							# 회원이메일
    member_grade: Mapped[int] = mapped_column(Integer)  									# 회원등급
    member_join_date: Mapped[datetime] = mapped_column(DateTime)  								# 회원가입일
    member_leave_date: Mapped[datetime] = mapped_column(DateTime)  								# 회원탈퇴일
    member_join_falg: Mapped[bool] = mapped_column(Boolean) 									# 회원탈퇴여부
    member_active: Mapped[bool] = mapped_column(Boolean)  									# 회원활성화여부
    member_terms_of_service: Mapped[bool] = mapped_column(Boolean) 							# 서비스이용약관동의여부
    member_privacy_policy: Mapped[bool] = mapped_column(Boolean) 							# 개인정보처리방침동의여부
    member_recive_marketing_info: Mapped[bool] = mapped_column(Boolean)						# 마케팅정보수신동의여부
    member_approve: Mapped[bool] = mapped_column(Boolean)  									# 관리자승인

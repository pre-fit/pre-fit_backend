...
class Shop_Point_Histories_Model(Base):
    ...
    point_update_date: Mapped[datetime] = mapped_column(DateTime)										# 포인트이력날짜
    point_status: Mapped[bool] = mapped_column(Boolean)												# 포인트상태이력(적립, 사용)
    point_use_amount: Mapped[int] = mapped_column(Integer)											# 포인트사용량
